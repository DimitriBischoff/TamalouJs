...
from tamalou.entities import Entity
...
class RoundState:
    ...
    def get_power(card):
        ...
        if  6 <= x <=  7: return POWER.SEE_MY_CARD
        if  8 <= x <=  9: return POWER.SEE_YOU_CARD
        if 10 <= x <= 11: return POWER.SWAP
        if x == 12:       return POWER.KING_PART_1
        return 0
    ...
    def swap(self, p1, c1, p2, c2):
        ...
        if (player1 and player2) and (0 <= c1 < 8) and (0 <= c2 < 8):
            ...
            if (self.counter == 0 \
                ...
                and current_player.power in {POWER.SWAP, POWER.KING_PART_1, POWER.KING_PART_2}):
    ...
    def next(self):
        ...
        current_player.power = POWER.NOTHING
        current_player.state = STATE.NOTHING
    ...
    def if_stock(self, auth_id, card_index):
        ...
        return self.is_current_player(auth_id) \
            ...
            and player.state in {STATE.GET_HEAP, STATE.GET_STACK}
    ...
    def if_pull(self, auth_id, choice):
        ...
        return auth_id == current_player.auth_id \
            and player.state == STATE.NOTHING \
    ...
    def if_discard(self, auth_id, card_index):
        ...
        base_rule = not self.discard_locked \
            and current_player.state != STATE.GET_HEAP \
            and self.counter > 0 \
            and len(self.heap) > 0
