...
class GameManager {
	...
	game_update(game) {
		...
		for (let i = 0; i < game.players.length; i++) {
			...
			this.dm.hand_update(this.players[p.id], (is_dblclick) => {
	...
	click_stack() {
		console.log('stack');
		if (this.in_game && this.players[this.my_id].state == 0) {
	...
	click_heap() {
		console.log('heap');
		if (this.in_game) {
			if (this.players[this.my_id].state == 0) {
				this.sc.send('pull', 'heap');
			}
			else if (this.players[this.my_id].state == 2) {
				this.sc.send('use');
			}
		}
	}
	...
	view_update(player_id) {
		this.dm.view_update(this.players[player_id], (card_index, is_dblclick) => {
			if (is_dblclick)
	...
	click_card(player_index, card_index) {
		...
		if (this.cards_selected[player_index] == card_index) {
			switch(this.players[this.my_id].state) {
				case 1: case 2: if (player_id == this.my_id) {
					this.sc.send('stock', card_index);
				}
				break;
				case 3: this.power(player_index, card_index);
				break;
				default:
			}
		}
			...
			switch (this.players[this.my_id].state) {
				...
				case 3: {
					switch (this.players[this.my_id].power) {
						case 1: case 5: if (player_id == this.my_id) {
							this.select_card(player_index, card_index);
						}
						break;
						case 2: case 4: if (player_id != this.my_id) {
							this.select_card(player_index, card_index);
						}
						break;
						case 3: {
	...
	power(player_index, card_index) {
		...
		switch(this.players[this.my_id].power) {
			...
			break;
			case 3: {
				console.log(Object.keys(this.cards_selected).length == 2 && my_index in this.cards_selected);
